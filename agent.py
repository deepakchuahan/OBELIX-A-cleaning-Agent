...
def _load_once():
    ...
    class D3RQN(nn.Module):
        def __init__(self):
            super(D3RQN,self).__init__();
            self.lstm=nn.LSTM(ins,64,batch_first=True);
            self.v_l=nn.Linear(64,1);
            self.a_l=nn.Linear(64,outs);
        def forward(self,x,h,c):
            v,(nh,nc)=self.lstm(x,(h,c));
            val=self.v_l(v);
            adv=self.a_l(v);
            mn_a=torch.mean(adv,dim=2,keepdim=True);
            q=val+(adv-mn_a);
            return q,nh,nc;
